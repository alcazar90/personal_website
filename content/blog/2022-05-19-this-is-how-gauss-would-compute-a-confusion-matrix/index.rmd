...
is a practical and conceptually simple tool for evaluating a classification model.
...
> _A confusion matrix is the matrix multiplication by the class' dimension between
the true and predicted labels, both encoding as one-hot vectors._

If we have the true labels of 4 observations in vector $\boldsymbol y = [1, 0, 2, 1]$,
their one-hot encoding will be:
...
0 & 0 & 1\\
0 & 1 & 0
\end{bmatrix}~\in~[0,1]^{4~x~3}
$$

Some classification model gives us the predicted label in vector $\hat{\boldsymbol y} = [2, 0, 2, 0]$,
...
$\boldsymbol T^{\top}\hat{\boldsymbol T}~\in~\boldsymbol Z_{0+}^{3x3}$. So again,
...
Mainly, two steps to get our confusion matrix.

First, we need a way to transform a vector $\boldsymbol v$ with k-classes in their one-hot-encoding
...
Second, compute the confusion matrix, $~\boldsymbol T^{\top}\hat{\boldsymbol T}~\in~\boldsymbol Z_{0+}^{KxK}~$, for k-classes; there are many ways of doing it with `numpy` as
you can see in following code. Below I used the canonical notation for name the
...
We are done! Of course, you can always get your confusion matrix from your
favourite store ;)
