...
It is necessary to clean before to collapse `raw_data` into one dataset. We want to discard rows with meta-information and select all the columns until column *"Valor Par (en miles de $)"*. So first we need to detect the row that contain the column names of the table in the spreadsheet, then the name of our last column, and finally the last row that contain values. We can do this by applying regular expressions on coordinate view and encapsulate all this process into the function `clean_spreadsheet` to apply simultaneously over each dataframe. 
...
Approximately a 86% of the data has 19 columns, then is make sense to focus on the case of 19 columns. The other two cases require to compare columns to find the missing and extra column but these details distract us from the main purpose. So we follow with the case of 19 columns to ilustrate the approach.
...
The result is a data with 21 columns and 33261 observations. We are almost already, put our attention into the variable `fecha_inscripccion`.
...
Finally we have a clean dataset ready to analyze. You can take a look to a few observations!
