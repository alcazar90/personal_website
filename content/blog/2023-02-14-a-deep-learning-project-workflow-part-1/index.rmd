---
title: A Deep Learning Project Workflow Part 1 -> HF datasets + W&B
author: Cristóbal Alcázar
date: '2023-02-22'
slug: []
categories: [workflow, deep-learning, W&B, HF]
tags: []
comments: no
showcomments: yes
showpagemeta: yes
...
I will use a project I started on fine-tuning an image segmentation model (i.e. <a href="https://arxiv.org/abs/2105.15203" target="_blank">SegFormer</a>) in cell images from a high-throughput microscope 🔬. The idea is to train a model using cell shots with mask labels that denote the ones alive. A good model can connect directly to the microscope and help scientists detect cells quickly affected by a given treatment.
...
1. Create files to separa code from `training.py`, such as `model.py` for
code related to download and load the model checkpoints, and `inference.py` for compute evaluation metrics
1. Use a cloud provider such as [Lambda Cloud](https://lambdalabs.com/service/gpu-cloud) to connect via ssh and run the training script. Check if the results save in W&B.
...
1. Study cases for popular deep learning code repositories such as [OpenAI whisper model](https://github.com/openai/whisper). 